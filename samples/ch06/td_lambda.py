...
def exec_episode(s: State, v: list[float]) -> list[(State, float, float)]:
    """
    エピソードを1回実行し, エピソード中に辿った状態の履歴と報酬, 状態価値関数の値を返す.
    """
    ...
    while not env.is_terminal(s):
        ...
        next_s = env.f(s, a)
        history.append((s, -env.r(next_s), -v[next_s.encode()]))
        s = next_s
...
def eval_policy(v: list[float], history: list[(State, float)]):
    g = 0.0
    for s, reward, v_next in reversed(history): # 履歴を逆向きに辿る
        s_idx = s.encode()
        g = reward + GAMMA * ((1 - LAMBDA) * v_next + LAMBDA * g)
